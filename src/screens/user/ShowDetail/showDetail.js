...
// Components
import { MovieCardSmallUser } from "../../../components/Card/MovieCardSmall";
import { LoadingScreen } from "../../../components/Loading/LoadingScreen";
import { PrimaryButton } from "../../../components/Button/PrimaryButton";
import { TertiaryButton } from "../../../components/Button/TertiaryButton";
import { BLACK, PRIMARY, SECONDARY, WHITE } from "../../../utils/styles/color";
...
const ShowDetailScreen = ({ route, navigation }) => {
    ...
    const RenderDateList = () => {
        ...
        return (
            <View style={ShowDetailStyle.gap}>
                <Text style={ShowDetailStyle.title}>Pick the date</Text>
    ...
    const RenderSeating = () => {
        ...
        // Seating Price
        for (let spindex = 0; spindex < selectedCinema.seatRow; spindex++) {
            nextRowPrice =
                selectedCinema.seatPrice[
                String.fromCharCode(rowName.charCodeAt(0) - 1)
                ];
            seatingPrice.push(
                <View style={ShowDetailStyle.seatAndPriceBox}>
            ...
            for (let colIndex = 1; colIndex <= selectedCinema.seatCol; colIndex++) {
                tempRowName = rowName + colIndex;
                {
                    reservedSeatNumbers.includes(tempRowName)
                        ? seatingRow.push(
                            <View style={ShowDetailStyle.seatGridBox}>
                                <View
                                    style={[
                        ...
                        : seatingRow.push(
                            <TouchableOpacity key={colIndex} onPress={() => clickSeat(rowIndex, colIndex)} style={ShowDetailStyle.seatGridBox}>
    ...
    const Booking = async () => {
        try {
            ...
            .then(() => {
                Alert.alert('User Booking Ticket Successfully');
            ...
            <View style={ShowDetailStyle.container}>
                <ScrollView showsVerticalScrollIndicator={false}>
                    <View style={ShowDetailStyle.headerComponent}>
                        <MovieCardSmallUser movie={movie} />
                        <RenderCinemaList />
                        <RenderDateList />
                        <RenderTimeList />
                    </View>
